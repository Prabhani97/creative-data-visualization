...
read_clean_sound <- function(path, target_sr = 22050, target_duration = 4) {
  ...
  max_val <- max(abs(samples), na.rm = TRUE)
  if (max_val > 0) {
  ...
  if (length(samples) > target_len) {
    samples <- samples[1:target_len]
  } else if (length(samples) < target_len) {
    samples <- c(samples, rep(0, target_len - length(samples)))
  }
...
-   **Someone who wants to run this visualization can start from the beginning at from this point because I am saving the combined soundscape. Since here I will be accessing this a combined soundscape to build my data set containing it's time, frequency, energy features. So basically, anyone can and move forward with the visualization from here onwards, without accessing the audio files too.**
...
spectrogram_df <- as.data.frame(amp)
spectrogram_df$freq <- freq
...
speech_city <- sound_data %>%
  ...
  mutate(
    ...
    freq_band = as.numeric(cut(freq, breaks = 40))
  ) %>%
  group_by(time_index, freq_band) %>%
  summarise(
...
window_data <- speech_city %>%
  ...
  mutate(
    window_x = freq_band + runif(n(), -0.25, 0.25),
...
# Next I am adding the Foreground layer (ground and few trees), this is specifically placed after buildings so it appears in front
  ...
  labs(
    title = "Speech City Skyline: Urban Soundscape Frame {round(frame_time,3)}",
...
I tried to add maximum number of frames to present a smoother motion, but when imcreasing it , longer render times occurr. Also, I kept a reasonable frames per second value for human naked eye to visualize the sound features via the skyline slowly.
